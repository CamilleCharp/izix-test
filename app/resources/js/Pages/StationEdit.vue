...
const props = defineProps({ station: Object, station_types: Object, locations: Object });

const form = ref({
  name: props.station.name,
  spot: props.station.spot,
  type_id: props.station.type.id,
  location_uuid: props.station.location.uuid,
});
...
const onFormSubmit = () => {
  ...
  router.put(route('stations_update', props.station.uuid), {
  ...
  <AuthenticatedLayout>
    <div class="h-full flex flex-col space-y-8 justify-between">
      <Form v-slot="$form" @submit="onFormSubmit" class="m-auto">
        <h1 class="text-xl">Edit Station</h1>
